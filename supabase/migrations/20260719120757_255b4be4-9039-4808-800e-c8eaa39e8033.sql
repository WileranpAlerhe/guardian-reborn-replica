...
INSERT INTO public.products (id, slug, name, description, image, category, old_price, price, sold, rating, affiliate_url, badges, coupon_text, brand, active, "order") VALUES
('ar-britania-baq2200b','ar-britania-baq2200b','Ar-Condicionado Split Britânia BAQ2200B','Split parede, silencioso, controle remoto, alta eficiência energética.','/assets/product-ac-britania.png','casa',249.9,159.9,2340,4.9,'https://shopee.com.br',ARRAY['destaque','oferta'],'Cupom PRATI10 aplicado','Britânia',true,0),
('fone-i12','fone-i12','Fone Bluetooth i12 Sem Fio','Áudio estéreo, case carregador, autonomia 4h.','','eletronicos',89.9,49.9,10200,4.8,'https://shopee.com.br',ARRAY['mais-vendido','oferta'],'Cupom PRATI10 aplicado','Genérico',true,1),
('smartwatch-d20','smartwatch-d20','Smartwatch D20 Y68 Relógio Inteligente','Monitor cardíaco, notificações, resistente à água.','','eletronicos',149.9,89.9,8700,4.7,'https://shopee.com.br',ARRAY['mais-vendido','oferta'],'Cupom PRATI10 aplicado','Y68',true,2),
('caixa-som-bluetooth','caixa-som-bluetooth','Caixa de Som Bluetooth Portátil','Som potente, bateria longa.','','eletronicos',199.9,139.9,6300,4.6,'https://shopee.com.br',ARRAY['oferta'],'Cupom PRATI10 aplicado','JBL Style',true,3),
('mini-liquidificador','mini-liquidificador','Mini Liquidificador Portátil USB','Recarregável USB, para vitaminas e sucos.','','casa',79.9,59.9,7100,4.7,'https://shopee.com.br',ARRAY['mais-vendido','oferta'],'Cupom PRATI10 aplicado','PratiHome',true,4),
('tenis-casual','tenis-casual','Tênis Casual Unissex Branco','Confortável, sola antiderrapante.','','moda',189.9,129.9,6400,4.8,'https://shopee.com.br',ARRAY['novo'],'Cupom PRATI10 aplicado','UrbanStep',true,5),
('perfume-importado','perfume-importado','Perfume Importado Feminino 100ml','Fragrância floral marcante.','','beleza',219.9,149.9,3200,4.9,'https://shopee.com.br',ARRAY['novo','oferta'],'Cupom PRATI10 aplicado','LaBelle',true,6);
...
CREATE POLICY "analytics_insert_all" ON public.analytics_events FOR INSERT TO anon, authenticated WITH CHECK (true);
...
SELECT c.id, p.id, row_number() OVER (PARTITION BY c.id ORDER BY p."order")
FROM public.collections c CROSS JOIN public.products p
WHERE c.slug='mais-vendidos' AND 'mais-vendido' = ANY(p.badges);

INSERT INTO public.collection_products (collection_id, product_id, position)
SELECT c.id, p.id, row_number() OVER (PARTITION BY c.id ORDER BY p."order")
FROM public.collections c CROSS JOIN public.products p
WHERE c.slug='novidades' AND 'novo' = ANY(p.badges);

INSERT INTO public.collection_products (collection_id, product_id, position)
SELECT c.id, p.id, row_number() OVER (PARTITION BY c.id ORDER BY p."order")
FROM public.collections c CROSS JOIN public.products p
WHERE c.slug='ofertas-relampago' AND 'oferta' = ANY(p.badges);
...
CREATE TABLE public.orders (
  ...
  raw_response jsonb, raw_webhook jsonb,
  paid_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
